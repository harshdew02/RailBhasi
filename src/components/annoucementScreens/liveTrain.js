...
export default function LiveTrain() {
  ...
  return (
    <SafeAreaView>
      ...
      {/* green */}
      {(trainSchedule.length > 0) && <Green cardData={trainSchedule[0]} />}
      <ScrollView>
        {/* <Green /> */}
        {(trainSchedule.length > 0) && trainSchedule.map((cardData, idx) => {

          if (idx > 0 && idx < trainSchedule.length - 4)
            return <Blue key={idx} cardData={cardData} />
        })}


      </ScrollView>
      {(trainSchedule.length > 0) && <Green cardData={trainSchedule[-3]} />}
